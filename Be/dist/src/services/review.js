...
const createReview = async (review) => {
    ...
    try {
        ...
        console.error(err);
        throw new appError_1.AppError("Failed to create review", 500, false);
    }
};
...
const getReviewsByProductId = async (product_id) => {
    try {
        ...
        const reviewOfProduct = { total_reviews, average_rating, Reviews: Object.values(reviewsMap) };
        return reviewOfProduct;
    }
    catch (err) {
        console.error(err);
        throw new appError_1.AppError("Failed to get reviews", 500, false);
    }
};
exports.getReviewsByProductId = getReviewsByProductId;
const removeReviewById = async (review_id, user_id) => {
    try {
        const pool = await (0, database_1.connectionDB)();
        const result = await pool
            .request()
        ...
        if (result.rowsAffected[1] === 0) {
            throw new appError_1.AppError("You are not allowed to delete this review", 403, true);
        }
    }
    catch (err) {
        console.error(err);
        if (err instanceof appError_1.AppError)
            throw err;
        throw new appError_1.AppError("Failed to delete review", 500, false);
    }
};
...
const getReviewById = async (review_id) => {
    try {
        ...
        console.error(err);
        throw new appError_1.AppError("Failed to get review", 500, false);
    }
};
...
const updateReview = async (review) => {
    ...
    try {
        ...
        if (updateResult.rowsAffected[0] === 0) {
            throw new appError_1.AppError("You are not allowed to update this review", 403);
        }
        ...
        await transaction.rollback();
        console.error(error);
        throw new appError_1.AppError("Failed to update review", 500, false);
    }
};
