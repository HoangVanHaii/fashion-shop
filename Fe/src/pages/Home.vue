...
const handleCart = (id: number) => {
    productDetail.value = flashSaleHomes.value?.Products[id] || null;
    if (productDetail) {
...
onMounted(async () => {
    await useVoucher.getTop4VoucherGlobal();
    vouchers.value = useVoucher.vouchers;

    await useFlashSale.getFlashSaleHome();
    flashSaleHomes.value = useFlashSale.flashSales;
    localStorage.setItem('excludeIdHome', flashSaleHomes.value?.id ? flashSaleHomes.value.id.toString() : "" );

    await useFlashSale.getTotalSoldFlashSaleByIdStore();
    totalSolds.value = useFlashSale.totalSolds;

    await useProduct.getProductBestSellerStore();
    productBestSeller.value = useProduct.productBestSellers;

    await useProduct.getProductLatestStore();
    productLatests.value = useProduct.productLatest;
...
const getSold = (productId : number) => {
  const soldItem = totalSolds.value.find(item => item.product_id === productId);
    ...
    <div class="container">
        ...
        <div class="promo-flashsale" v-if="displayedProducts.length > 0">
            ...
            <div class="flashsale-title">
                <p >ĐỘC QUYỀN ONLINE GIẢM SỐC 70% ++</p>
                        ...
                        <div class="product-info">
                            ...
                            <div class="product-bottom">
                                <div class="grid-prices">
                                        <span class="grid-price-new">{{ product.flash_price ? formatPrice(product.flash_price!) : product.min_price }}</span>
                                        <span class="grid-price-old" v-if="product.flash_price">{{ formatPrice(product.max_price)}}</span>
                                </div>
                            ...
                            <div class="product-sold">
                                ...
                                <div class="progress-bar">
                                    <div  
                                        class="progress" 
                                        :style="{ width: Math.min((getSold(product.id) / product.sold_quantity) * 100, 100) + '%' }">
                                     </div>
            ...
            <button class="btn-best-seller" @click="btnShowMoreProductSale">
                    ...
                    <i :class="showAll ? 'fa-solid fa-angle-up' : 'fa-solid fa-arrow-right'"></i>
                </button>
        ...
        <div class="featured-products">
            <div class="best-sellers" v-if="productBestSeller.length > 0">
                ...
                <div class="products-grid bestseller" :class="showMoreBestSeller ? 'flex-wrap' : 'no-flex-wrap'">
                    <div
                        ...
                        <div class="grid-description">
                            ...
                            <div class="grid-info">
                                ...
                                <div class="grid-bottom">
                                    <div class="grid-prices">
                                        <span class="grid-price-new">{{ product.flash_price ? formatPrice(product.flash_price!) : formatPrice(product.min_price) }}</span>
                                        <span class="grid-price-old" v-if="product.flash_price">{{ formatPrice(product.max_price)}}</span>
                                    </div>
                ...
                <button class="btn-best-seller" @click="btnShowMoreBestSeller">
                    ...
                    <i :class="showMoreBestSeller ? 'fa-solid fa-angle-up' : 'fa-solid fa-arrow-right'"></i>
                </button>

            </div>
            <div class="new-arrivals" v-if="productLatests.length > 0">
                <span class="title">SẢN PHẨM MỚI RA MẮT</span>
                <div class="products-grid arrivals" :class="showMoreNewArrivals ? 'flex-wrap' : 'no-flex-wrap'">
                    <div
                ...
                <button class="btn-new-arrival" @click="btnshowMoreNewArrivals">
                    ...
                    <i :class="showMoreNewArrivals ? 'fa-solid fa-angle-up' : 'fa-solid fa-arrow-right'"></i>
                </button>
            </div>
        </div>
        ...
        <AddToCart 
            v-if="showFormAdd && productDetail"  :product="productDetail"
...
.btn-best-seller:hover, .btn-new-arrival:hover, .view-now:hover{
    background: #000;
    color: #fff;
}
.btn-best-seller i{
...
@media (max-width: 1200px){
    ...
    .promo-product.row-mode, .promo-product.grid-mode{
        gap: 14px;
    }
    .promo-product .product-item{
        width: 190px;
