...
const handleSubmit = async () => {
    ...
    if (!password.value || password.value.length < 4) {
        ...
        setTimeout(() => {
            textToast.value = '❌ Mật khẩu không hợp lệ'
        }, 0);
        return;
    }
    if (password.value != confirmPassword.value) {
        showNotification.value = false;
        setTimeout(() => {
            textToast.value = '❌ Mật khẩu không trùng khớp'
        }, 0);
        return;
    }
    sending.value = true;
    await auth.resetPasswordStore(props.email, confirmPassword.value);
    sending.value = false;
    if (auth.error) {
        textToast.value = '❌ không thể đổi mật khẩu'
        showNotification.value = false;
        return;
    }
...
<template>
  ...
  <div class="reset-password-container"  @click="handleClose">
    <div class="reset-password-card" @click.stop>
      ...
      <h2 class="title">Cài đặt mật khẩu mới</h2>
      <p class="description">
        Vui lòng cài đặt mật khẩu mới gồm 4-18 kí tự 
      </p>
